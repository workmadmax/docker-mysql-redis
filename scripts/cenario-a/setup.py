...
#    Updated: 2026/03/26 15:20:52 by mdouglas         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
cursor.execute("SELECT id, titulo, descricao, categoria FROM item_vetorial") #executo a querry
rows = cursor.fetchall() # pego os resultados da querry e guardo na variavel rows

def generate_qdrant_points(rows):
    points = []
    ...
    return (points);
points = generate_qdrant_points(rows);
qdrant.upsert(collection_name=COLLECTION, points=points) # update ou insert 'upsert'
